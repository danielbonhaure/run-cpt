...
RUN R -e "options(warn=2); install.packages('ggiraph', repos=${CRAN_MIRROR}, verbose=T, quiet=T, keep_outputs='/tmp/')"
...
#        --volume $(pwd)/plot.yaml:/opt/pyCPT/plot.yaml \
#        pycpt:latest /usr/local/bin/python /opt/pyCPT/main.py
...
#        --volume $(pwd)/config.yaml:/opt/pyCPT/config.yaml \
#        --volume $(pwd)/plot.yaml:/opt/pyCPT/plot.yaml \
#        pycpt:latest /usr/bin/Rscript /opt/pyCPT/plot.R
